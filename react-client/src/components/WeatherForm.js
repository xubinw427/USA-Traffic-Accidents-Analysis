  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (responseData["pred"] === "0") {
        responseMessage = `The Expected Delay is 0-15 minutes. (Prediction confidence: ${(
          responseData["prob"] * 100
        ).toFixed(2)}%)`;
      } else if (responseData["pred"] === "1") {
        responseMessage = `The Expected Delay is >15 minutes. (Prediction confidence: ${(
          responseData["prob"] * 100
        ).toFixed(2)}%)`;
      } else {
